	...
	b.prototype = {
				...
				a.each(g.data, function(h, j) {
					...
					if (j.type === "folder") {
						...
						if ("additionalParameters" in j && "item-selected" in j.additionalParameters && j.additionalParameters["item-selected"] == true) {
							i.find(".tree-folder-header i:eq(1)").removeClass(e.options["unselected-icon"]).addClass(e.options["selected-icon"]);
							i.find(".tree-folder-header:eq(0)").addClass("tree-folder-selected");							
							setTimeout(function(){
								i.find(".tree-folder-header").trigger("click");
							}, 1);
						}
					} else {
						if (j.type === "item") {
		...
		selectFolder: function(e) {
			...
			if (d.find("." + this.options["close-icon"]).length) {
				...
				f.find(".tree-folder-header i:eq(1)").removeClass(this.options["unselected-icon"]).addClass(this.options["selected-icon"]);
				f.find(".tree-folder-header:eq(0)").addClass("tree-folder-selected");
				this.$element.trigger("opened", d.data());
//				f.find(".tree-item").addClass("tree-selected");
			} else {
				...
				f.find(".tree-folder-header i:eq(1)").removeClass(this.options["selected-icon"]).addClass(this.options["unselected-icon"]);
				f.find(".tree-folder-header:eq(0)").removeClass("tree-folder-selected");
//				f.find(".tree-item").removeClass("tree-selected");
				this.$element.trigger("closed", d.data());
			}
		},
		selectedItems: function() {
			var treeFolderSelected = "";
			if(this.options["folderSelect"]){
				treeFolderSelected = ".tree-folder-selected,";
			}
			var e = this.$element.find(treeFolderSelected + ".tree-selected");
			var d = [];
